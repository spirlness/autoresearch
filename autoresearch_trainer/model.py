...
def target_tok_per_sec_for_mfu(peak_flops, num_flops_per_token, target_mfu_percent):
    if (
        peak_flops is None
        or peak_flops <= 0
        or num_flops_per_token <= 0
        or target_mfu_percent <= 0
    ):
        return None
...
class Block(nn.Module):
    ...
    def forward(
        ...
        mlp_input = norm(x)
        if (
            self.activation_checkpoint == "mlp_only"
            and self.training
...
class GPT(nn.Module):
    def __init__(self, config, attention_op):
        ...
        if config.activation_checkpoint not in {"none", "mlp_only"}:
            raise ValueError(
                f"Unsupported activation checkpoint mode: {config.activation_checkpoint}"
            )
        ...
        self.transformer = nn.ModuleDict(
            {
                ...
                "h": nn.ModuleList(
                    [Block(config, i, attention_op) for i in range(config.n_layer)]
                ),
            }
        )
    ...
    def _compute_window_sizes(self, config):
        ...
        window_sizes = [
            char_to_window[pattern[layer_idx % len(pattern)]]
    ...
    def forward_trunk(
        self, x: torch.Tensor, idx: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor
    ) -> torch.Tensor:
        cos_sin = cos, sin
        x0 = x
